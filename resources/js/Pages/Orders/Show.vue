...
const refreshStatus = async () => {
    ...
    try {
        const response = await fetch(route('api.payment.status', props.order.order_number), {
        ...
        if (data?.status === 'ok') {
            router.reload({ only: ['order', 'transaction', 'statusTimeline'] });
        }
    } finally {
                    ...
                    <div class="flex flex-col md:flex-row md:items-center md:justify-between gap-4">
                        ...
                        <div class="flex items-center gap-3">
                            <button 
                                ...
                                class="text-xs border border-black/20 hover:border-black px-3 py-1.5 transition-colors flex items-center gap-1.5 disabled:opacity-50"
                            >
                                <svg v-if="isRefreshing" class="w-3 h-3 animate-spin" fill="none" viewBox="0 0 24 24">
